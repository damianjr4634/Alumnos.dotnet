...
| [FrmEsba.pas](./Esba.Delphi%20XE2/ESBA/FrmEsba.pas) / `.dfm` | Formulario principal (`TFormEsba`) | **Pantalla central**: arriba un **menú general dxBar** por funcionalidad (Alumno, Listados, Regularización, Faltas...) con botones habilitados según `MNUOPC` de `YYY_BARRA_SEGU(codusu, 'ALUMNOS')`; en el centro el **buscador global de alumnos** (sin importar carrera: apellido/código/mail) con grilla (`MtAlumnos`, kbmMemTable cargada en background con `TFISQLThread`) y panel "ALUMNO SELECCIONADO" con las acciones sobre ese alumno. Al entrar a una opción que lo requiere, se elige la carrera (lista en `BarraCarrerasItems`, cargada de `YYY_BARRA_SEGU`). *Nota: la navegación agrupada por carrera (`Carreras.pas`/`dxNavBar`) fue el diseño anterior, descartado por incómodo con muchas carreras.* Persiste configuración visual por usuario. |
