...
uses
  ...
  strutils, Variants, OleServer, ExcelXP, Vcl.FileCtrl;

type
  TFrmlstactas = class(TForm)
    GroupBox1: TGroupBox;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
{$R *.dfm}
procedure TFrmlstactas.FormActivate(Sender: TObject);
begin
 Comision.SetFocus;
end;

procedure TFrmlstactas.ImprimirClick(Sender: TObject);
var
  largopagina:real;
  contareg:integer;
  sw : integer;
 ...
 If (CodigoMateria.Text = '') and (Comision.Text<>'') then
  Begin
     SqlComi.SQLs.SelectSql.Text:= 'SELECT TRIM(D.DOCENTE) AS DOCENTE, TRIM(M.DESCRIPCI) AS DESCRIPCI, C.CUTUCO, C.COD_MAT '+
                        ...
                        'WHERE C.CARRE = '+#39+VCarrera+#39+' AND C.CUTUCO = :CUTUCO  AND  C.CUA_ANIO = '+#39+AnsiReplaceText(Cuatrimestre.Text,'/','')+#39+'  ORDER BY 3,4';
 ...
 If (CodigoMateria.Text <> '') and (Comision.Text<>'') then
   Begin
     SqlComi.SQLs.SelectSql.Text:= 'SELECT TRIM(D.DOCENTE) AS DOCENTE, TRIM(M.DESCRIPCI) AS DESCRIPCI, C.CUTUCO, C.COD_MAT '+
                        ...
                        'C.COD_MAT = :MATERIA  ORDER BY 3,4';
 ...
 If (CodigoMateria.Text <> '') and (Comision.Text='') then
   Begin
     SqlComi.SQLs.SelectSQL.Text:= 'SELECT TRIM(D.DOCENTE) AS DOCENTE, TRIM(M.DESCRIPCI) AS DESCRIPCI, C.CUTUCO, C.COD_MAT '+
                        ...
                        'LEFT OUTER JOIN DOCENTES D  ON C.CODPROFES = D.CODPROFES '+
                        'LEFT OUTER JOIN MATERIAS M  ON C.COD_MAT = M.CODMATERI AND C.CARRE = M.CODCARRE '+
                        'WHERE C.CARRE = '+#39+VCarrera+#39+' AND C.CUA_ANIO = '+#39+AnsiReplaceText(Cuatrimestre.Text,'/','')+#39' AND C.COD_MAT = :MATERIA  ORDER BY 3,4';
     SqlComi.ParamByName('MATERIA').AsString := codigomateria.Text;
   End;
 SqlComi.Active := True;
 SqlComi.FetchAll;
 ...
 Else
  If PrintDialog1.execute then
   Begin
    ...
    Imagen.LoadFromFile(vUnidad+CARPETA_PLANTILLA+'\actas.wmf');
    ...
    While not SqlComi.eof do
     begin
      ...
      FormPreview.VistaPrevia1.Canvas.TextOut(1,LargoPagina,'Con asistencia del Sr/a.Profesor/a: '+(SqlComi.FieldByName('DOCENTE').AsString)+' se procedió a cumplir con el resultado que se consigna a continuación',GmCentimeters);
      ...
      While (not SqlDatos.eof) and  (largopagina<=28)  do
       begin
           ...
           FormPreview.VistaPrevia1.Canvas.Font.Size := 8;
           FormPreview.VistaPrevia1.Canvas.Font.Style := [];
           FormPreview.VistaPrevia1.Canvas.TextOut(2,largopagina,SqlDatos.FieldByName('COD_ALU').AsString,gmcentimeters);
           FormPreview.VistaPrevia1.Canvas.TextOut(5.1,largopagina,SqlDatos.FieldByName('APELLIDO').AsString + ', ' + SqlDatos.FieldByName('NOM_APE').AsString ,gmcentimeters);
        ...
        if (not sqlcomi.Eof) or (largopagina>28) then begin
          ...
          FormPreview.VistaPrevia1.Canvas.StretchDraw(0,0,21.54,35.56,Imagen,GmCentimeters);
//          FormPreview.VistaPrevia1.Canvas.StretchDraw(1,1,21,35,Imagen,GmCentimeters);
        end;
        LargoPagina := 1.5;
...
procedure TFrmlstactas.salirClick(Sender: TObject);
begin
  Close
end;


procedure TFrmlstactas.OpcionesClick(Sender: TObject);
begin
  PrintSetup.Execute;
end;

procedure TFrmlstactas.comisionKeyPress(Sender: TObject; var Key: Char);
begin
 if key=#13 then
  Cuatrimestre.SetFocus;
end;

procedure TFrmlstactas.cuatrimestreKeyPress(Sender: TObject;
  var Key: Char);
begin
 if key=#13 then
...
procedure TFrmlstactas.CodigoMateriaKeyPress(Sender: TObject;
  var Key: Char);
begin
 if key=#13 then
    Imprimir.SetFocus;
...
procedure TFrmlstactas.BtnExcelClick(Sender: TObject);
Var
  FrmProgress: TForm;
  Barra : TProgressBar;
 ...
 Else begin
     ...
     While not SqlComi.eof do begin
         ...
         Except
            ...
            Exit;
         End;
         Libro:=AppExcel.Workbooks.Open(vUnidad+CARPETA_PLANTILLA+'\Planilla_de_notas_terciaria.xls',EmptyParam,true,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,0);
         SqlDatos.SQLs.SelectSQL.Text:='SELECT DISTINCT A.COD_ALU, TRIM(A.APELLIDO) AS APELLIDO, TRIM(A.NOM_APE) AS NOM_APE, TRIM(C.CONDICION) AS CONDICION, C.CUTUCO, A.COD_ALU '+
                                       'FROM  CURSADA C '+
                                       'LEFT OUTER JOIN ALUMNOS A  ON C.COD_ALU = A.COD_ALU AND C.CARRE=A.CARRE '+
                                       'WHERE C.CUTUCO = '+IntToStr(SqlComi.FieldByName('CUTUCO').AsInteger)+' AND C.CUA_ANIO = '+#39+Cuatrimestre.Text+#39+
...
procedure TFrmlstactas.CodigoMateriaButtonClick(Sender: TObject);
Var b:Variant;
begin
...
procedure TFrmlstactas.CodigoMateriaExit(Sender: TObject);
Var b:Variant;
begin
