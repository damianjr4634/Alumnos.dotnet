...
uses
  ...
  Grids, DBGrids, StdCtrls, Buttons, ComCtrls, Datamodule, DB,
  RXToolEdit, Mask, RXCurrEdit,
     ...
     = IDOK Then
       Begin
         ...
         If (Nota1.value >= 6) And (Nota1.value <= 10)Then
           Begin
            If (DataModule.CustomerData.IBdsVarios.FieldByName('CONDICION').AsString = 'PREVIO') or (DataModule.CustomerData.IBdsVarios.FieldByName('CONDICION').AsString = 'PREVIA') Then
             Begin
             ...
             Begin
                ...
                DataModule.CustomerData.IBupVarios.ParamByName('CONDICION').AsString := 'LIBRE'
             End
            else If (DataModule.CustomerData.IBdsVarios.FieldByName('CONDICION').AsString = 'P/EQUIVALEN') or (DataModule.CustomerData.IBdsVarios.FieldByName('CONDICION').AsString = 'FINAL') Then
             Begin
                MessageDlg(Pchar(VNombreUsuario + ', el alumno quedo en condicion de Regular'),mtInformation, [mbok],0,mbok);
         ...
         Datamodule.CustomerData.IBdsVarios.ParamByName('USUARIO').value:=funcionesConfiguracion.codusu;
...
begin
  ...
  If (VCarrera = 'BAC') Or (VCarrera = 'CNA') or (vcarrera='333') or (vcarrera='650') or (VCarrera<>'197916') Then
     Begin
         Nota1.Enabled  := True;
         Nota2.Visible  := False;
         Nota3.Visible  := False;
  ...
  End
  else If (Carreras.CarreIndexOf(VCarrera).tipo='TER') Then Begin
