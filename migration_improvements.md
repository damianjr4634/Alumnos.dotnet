...
|---|---|
| `FrmEsba` (MDI-like, barra dxBar dinámica persistida en `.bar` por usuario) | `MainLayout.razor` con `MudAppBar` + `MudDrawer`: menú de navegación agrupado por carrera (sucesor de `Carreras.pas` / `dxNavBar`), generado desde datos y **filtrado por los permisos del usuario** (claims/políticas, sucesor de `BARRA_SEGU`). La personalización por usuario (`.bar`/`.not`) se guarda en BD como preferencias, no en archivos locales. |
