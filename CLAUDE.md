...
| 14 | **Notas de finales y actas** (diferido del hito 8): carga de notas de final ✅ 2026-06-26 — por mesa/comisión (`/examenes/notas-finales`) y por alumno (`/alumnos/{c}/{cod}/notas-finales`, desde el buscador). Decisión: portado a C# **sin staging** — `XXX_MESAS`/`XXX_CARGA_FINAL` reescritos en `CargaFinalRepository` (flag `consumirPermiso`: por mesa lo borra, por alumno no) + `CalculoCondicionFinal`; el buffer `"$$$PERMEXA"` se reemplaza por estado del componente; test de equivalencia contra `XXX_MESAS` en verde. **Pendiente:** actas (mesas, A-REGULAR, reincorporación) | 🟡 parcial |
