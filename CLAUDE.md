...
| 12 | **Endurecimiento para producción** (deploy en **contenedor Docker gestionado por Portainer**: config y secretos por variables de entorno del stack). Desglosado en 6 incrementos por riesgo: **12.1 ✅ 2026-07-02** config por env vars + DataProtection keys persistidas en volumen — `sysdba/masterkey` fuera del repo (user-secrets en dev), `ConnectionStrings__Esba` por env (sin código de parsing) y `DataProtection__KeysPath` (en Production el arranque falla sin ella: sin claves persistidas cada redeploy invalida cookies/sesiones); env vars documentadas en `docs/migracion/hito12-endurecimiento.md` · **12.2** usuario Firebird de mínimos privilegios (inventariar tablas/SP accedidos; usuario/host por env) · **12.3** autorización por políticas 🔴: mapear `MNUOPC` de `YYY_BARRA_SEGU` a `[Authorize(Policy=…)]` por área/carrera — **el servidor deniega, la UI solo oculta**; datos ya modelados en hito 10, falta enforcement + filtrar menú por las mismas policies · **12.4** sesión única en middleware 🟡 (token por usuario tipo `USUARIOS.UID`/`CheckCodSecion`; login nuevo invalida circuito anterior; depende de DataProtection estable de 12.1) · **12.5** Serilog + manejo global de excepciones 🔴 (sink **consola** para Portainer, `CorrelationId` por request/circuito, error genérico al usuario) · **12.6** deploy: **Dockerfile multi-stage (.NET 10) + stack Portainer** (env vars + volúmenes DataProtection/logs). **A confirmar:** ¿Firebird como contenedor en el mismo stack o servidor externo/host? Deuda transversal a absorber: foto del alumno (endpoint autenticado, cuelga de 12.3/12.6), `CARRE_GRP` (junto a 12.3), modo claro/oscuro persistido, adjuntos+firma HTML del correo por comisión (assets por usuario, como la foto) | ⬜ |
| 13 | **Fase 5 — retiro de SPs `XXX_*`**: portar PSQL a C# con tests de equivalencia, un SP por vez (empezar por los de mayor riesgo/uso) | ⬜ |
| 14 | **Notas de finales y actas** (diferido del hito 8): carga de notas de final ✅ 2026-06-26 — por mesa/comisión (`/examenes/notas-finales`) y por alumno (`/alumnos/{c}/{cod}/notas-finales`, desde el buscador). Portado a C# **sin staging** — `XXX_MESAS`/`XXX_CARGA_FINAL` reescritos en `CargaFinalRepository` + `CalculoCondicionFinal`; el buffer `"$$$PERMEXA"` se reemplaza por estado del componente. **Actas** ✅ 2026-06-30 — las 4: volante de mesa (`XXX_MESAS_ALUMNOS`) + A/REGULAR, reincorporación y exámenes (COMARM+CURSADA), en **PDF Oficio (QuestPDF) + Excel (ClosedXML)**, `/examenes/actas/{mesa,comision}`; decode de CUTUCO (CUatrimestre-TUrno-COmisión) en `CodigoComision`; equivalencia contra el SQL legacy en verde (ver `docs/migracion/hito14-actas.md`) | ✅ 2026-06-30 |
| 15 | **Regularización de materias** (hueco detectado 2026-06-26 — cierra el flujo académico desde el alta del alumno): carga de las notas del **cursado** (parciales/TP/recuperatorios: `TP_EVA`/`RECUP`/`TP_EVA2`/`RECUP2`/`TP_EVA3`/`REGULAR`/`PROM` de `CURSADA`) y resolución de la **condición** del alumno en la materia (REGULAR → habilita rendir final; promoción/aprobado → va directo a `ANALITIC`; LIBRE/RECURSA). Dos variantes como el legacy: **por alumno** (`RegularizacionDeMaterias_nuevo.pas`) y **por comisión, en lote** (`RegularizacionDeMateriasXComision_nuevo.pas`). El modelo de datos ya existe (hito 3); lógica en SPs `XXX_REGULARIZACION_MAT_*` (`_TERC`/`_BAC`/`_333`/`_POSTVAL`) + `XXX_FECHA_REGULARIZACION` → wrappers 2.B (o port a C# con la misma estrategia que el hito 14). Es **prerrequisito** del permiso de examen (hito 8) y de la nota de final (hito 14): completa la cadena alta → inscripción → cursado/asistencias → **regularización** → permiso → final → analítico. Habilita la opción de menú `Alumnos › Regularización` (hoy deshabilitada). **Decisión 2026-06-30: portado a C# (como hito 14, sin staging `"$$$CURSADA"`), arrancando por terciarias.** Terciarias ✅ 2026-06-30 — `CalculoCondicionRegularizacionTerciaria` (dominio, equivalencia contra `XXX_REGULARIZACION_MAT_TERC`) + `RegularizacionRepository` (rama TER del commit) + por alumno (`/alumnos/{c}/{cod}/regularizacion`, acción del buscador) y por comisión (`/academica/regularizacion-comision`, menú habilitado). **Bachillerato ✅ 2026-07-01** — `CalculoCondicionRegularizacionBachiller` fusiona `_BAC` (faltas) + `_POSTVAL` (ladder de notas; CONSEJO interactivo resuelto con selector inline Consejo/Regular/Libre + botón "Libre"), equivalencia contra ambos SP; `RegularizacionRepository.ConfirmarBachilleratoAsync` (rama BAC del commit → CURSADA_HST+ANALITIC si REGULAR). Alcance = carrera `BAC` (única con guard `CARRE='BAC'` en `_POSTVAL`); CNA/secundario diferidos. **Equivalencia del commit ✅ 2026-07-01** — `RegularizacionCommitEquivalenciaTests` compara el volcado C# (`ConfirmarFilas*Async`) vs `XXX_REGULARIZACION` sobre CURSADA/CURSADA_HST/ANALITIC (TER PROMOCIONA + BAC REGULAR + BAC update-only); destapó y corrigió 2 divergencias del port terciario (CONDANT en TER lo deja NULL el SP; `SqlFechaPromocion` truncaba `@CuaAnio`). **Secundario 333/650 ✅ 2026-07-01** — `CalculoCondicionRegularizacion333` (port de la lógica *activa* de `_333`: 2° trimestre / exámenes dic-mar → REGULAR/ENPROCESO/PREVIA + `NOTAFIN`/`NOTAFIN_FECHA`; el bloque PROM del SP estaba comentado) + `RegularizacionRepository.Confirmar333Async` (rama 333/650 del commit) + acción "A previa"; equivalencia de condición y de commit. **CNA ✅ 2026-07-01** — sin SP: la condición se decide por la nota final (`CalculoCondicionRegularizacionCna`: ≥7 REGULAR / ≥1 RECURSA / si no CURSANDO) + `RegularizacionRepository.ConfirmarCnaAsync` (rama BAC del commit); equivalencia del commit. `197916` (`TIPO=BAD`) queda fuera (flujo no deducible, a confirmar). Ver `docs/migracion/hito15-regularizacion.md`. Deuda transversal: prefill de faltas (`XXX_CONT_FALTAS`) | ✅ 2026-07-01 |
...
prefill de faltas de la regularización desde `XXX_CONT_FALTAS` cuando CURSADA está en 0 (hito
15: hoy se leen de CURSADA y el operador las edita) · regularización de la carrera `197916`
(`TIPO=BAD`): flujo no deducible del código legacy, a confirmar con el usuario.
